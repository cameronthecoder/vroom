...
export class UserService {
    ...
    async createInitalUserIfNoneExist(user: Insertable<Users>): Promise<User | undefined> {
        console.log("No users found, creating initial user...");
        user.password_hash = await hashPassword(user.password_hash);
        const newUser = await this.db.insertInto('users').values(user).returningAll().executeTakeFirst();
        return newUser;
    }
}